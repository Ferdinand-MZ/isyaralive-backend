...
async def get_dictionary_meaning(entry_id: int, db: Session = Depends(get_db)):
    ...
    Kolom `meaning` di kamus masih kosong untuk hampir semua entri (kamus
    kita fokus ke video peraga). Supaya layar ini tidak selalu tampil
    "Penjelasan belum ditulis", penjelasannya disusun `makna_service`
    (definisi KBBI yang dirapikan Asisten AI) saat kosong, lalu DISIMPAN ke
    kamus supaya kunjungan berikutnya tidak perlu memanggil jaringan lagi.

    `illustration_path` tidak pernah diisi dari internet — gambar hanya berasal
    dari aset yang disiapkan admin.

    Konten kurasi editorial tetap menang: kalau kolomnya sudah diisi admin,
    KBBI maupun AI tidak pernah dipanggil.
    """
    entry = db.query(DictionaryEntry).filter(DictionaryEntry.id == entry_id).first()
    if not entry:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Kata tidak ditemukan di kamus")
    ...
    if not (entry.meaning or "").strip():
        penjelasan = await cari_makna(entry.word)
        if penjelasan:
            entry.meaning = penjelasan["makna"]
            entry.source = penjelasan["sumber"]
            db.commit()
            db.refresh(entry)
